...
# Given that 20 cent rewards demand constant acceptances, we expect the proportions to be constant for these choices
# This gives an error in the t.test due to the lack of variability and normal errors, so we use try to bypass this issue
splitData_tests <- sapply(splitData, function(data) try(t.test(data$propComplete, mu = unique(data$Optimal))$p.value))

# Adjust for multiple comparisons using BY-FDR
trueProp$Deviation_pval <- round(p.adjust(splitData_tests, method = "BY"), digits = 2)
trueProp$Deviation_pval[is.na(trueProp$Deviation_pval)] <- 1
