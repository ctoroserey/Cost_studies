...
plotLogis <- function(coeffs = 1, group = "Cognitive", handling = 1:14, reward = 1:20, xAxis = "Offer", plotType = lines, color = "black", lType = 1, pchType = 16){
...
# # code to calculate the rwd rate for each acceptance threshold per handling
# rwd <- c(4, 8, 20)
# handling <- c(14, 10, 2)
# travel <- c(2, 6, 14)
...
autoplot(survData) +
  ...
  ylim(0, 1) +
  labs(x = "Time in Trial", y = "Proportion Completed", fill = "Cost", color = "Cost") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
        axis.line = element_line(colour = "black"),
...
#   )) %>%
#   group_by(SubjID, Cost, Handling, Offer, BlockOrder) %>%
#   summarise(pAccept = mean(Choice)) %>%
#   ggplot(aes(interaction(Offer, Handling), pAccept, linetype = BlockOrder, color = BlockOrder)) + 
#     geom_point(size = 3) + 
#     #geom_errorbar(aes(ymin = meanComplete - SE, ymax = meanComplete + SE), width = 0.2, size = 1) +
...
#     Handling[1] == 14 ~ "Fourteen",
#   )) %>%
#   group_by(Cost, Handling, Offer, FirstBlock) %>%
#   summarise(pAccept = mean(Choice),
#             SE = sd(Choice) / sqrt(length(Choice))) %>%
#   ggplot(aes(interaction(Offer, Handling), pAccept, color = Cost)) + 
#     geom_point(size = 3) + 
...
#     Handling[1] == 14 ~ "Fourteen",
#   )) %>%
#   group_by(Cost, Handling, Offer, FirstBlock, Half) %>%
#   summarise(pAccept = mean(Choice),
#             SE = sd(Choice) / sqrt(length(Choice))) %>%
#   ggplot(aes(interaction(Offer, Handling), pAccept, color = Cost)) + 
#     geom_point(size = 3) + 
