...
optimizeOCModel <- function(Data, Algorithm = "NLOPT_LN_NEWUOA", simplify = F) {
  # Prep data
...
propAov <- aov(pComplete ~ factor(Cost), data = temp)
...
# plots
p1 <- ggplot(aes(pComplete, color = Cost), data = temp) +
...
prop.aov <- with(temp, aov(pComplete ~ factor(Cost) * Handling * Reward + 
...
trueProp <- dataAll %>%
        ...
        summarise(meanComplete = mean(pComplete),
                  SE = sd(pComplete) / sqrt(length(pComplete)))
...
# 
temp <- lapply(mixLogis_main, function(model) {as.list(summary(model)$coefficients[2:4, 1])})
mixCoeffs <- as.matrix(bind_rows(temp)) #reduce(temp, full_join) 
dimnames(mixCoeffs) <- list(names(temp), substr(colnames(mixCoeffs), 5, 20))
mixCoeffs <- as.matrix(mixCoeffs[, match(rownames(mixCoeffs), colnames(mixCoeffs))])
mixCoeffs[is.na(mixCoeffs)] <- 1
mixCoeffs <- mixCoeffs[c("Cognitive", "Physical", "Wait", "Easy"), c("Cognitive", "Physical", "Wait", "Easy")]

# matrix of pvals
temp <- lapply(mixLogis_main, function(model) {as.list(summary(model)$coefficients[2:4, 4])})
mixPvals <- as.matrix(bind_rows(temp)) 
dimnames(mixPvals) <- list(names(temp), substr(colnames(mixPvals), 5, 20))
mixPvals <- as.matrix(mixPvals[, match(rownames(mixPvals), colnames(mixPvals))])
rownames(mixPvals) <- colnames(mixPvals)
mixPvals <- mixPvals[c("Cognitive", "Physical", "Wait", "Easy"), c("Cognitive", "Physical", "Wait", "Easy")]
diag(mixPvals) <- 1
...
This plot shows the AIC for every model. The figure below shows AIC and BIC for all the models. As predicted, the model with all main effects reduced the deviance the most ($R^2$ = `r round(mixLogis_compare$Rsq, digits = 2`).
...
propFails <- dataAll %>% 
  filter(Cost != "Wait") %>% 
  group_by(SubjID, Cost) %>% 
  summarise(propFails = mean(Choice == 2)) %>%
  group_by(Cost) %>% 
...
(RTs <- dataAll %>%
  ...
  filter(Choice < 2) %>% # change this to 1 to reproduce the original plot, since it would only plot the cdf of quits
  ggplot(aes(RT, color = Cost)) + 
    ...
    labs(x = "Reaction Time", y = "Proportion Accepted") +
    theme(panel.grid.major = element_blank(),
      panel.grid.minor = element_blank(),
      panel.background = element_blank(),
      axis.line = element_line(colour = "black"),
...
colnames(summary_prepost) <- c("Intercept", "Coefficient", "Int_CI-low", "Int_CI-high", "Beta_CI-low", "Beta_CI-high")
```
