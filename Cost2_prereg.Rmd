...
# Add an AR predictor based on the number of quits
waitData <- lapply(waitData, function(data) data %>% mutate(AR = seqQuits(Choice)))
cogData <- lapply(cogData, function(data) data %>% mutate(AR = seqQuits(Choice)))
physData <- lapply(physData, function(data) data %>% mutate(AR = seqQuits(Choice)))
easyData <- lapply(easyData, function(data) data %>% mutate(AR = seqQuits(Choice)))

# Logistic fits
waitLogis <- lapply(waitData, function(data) glm(Choice ~ Handling + Reward, data = data, family = "binomial"))
cogLogis <- lapply(cogData, function(data) glm(Choice ~ Handling + Reward, data = data, family = "binomial"))
physLogis <- lapply(physData, function(data) glm(Choice ~ Handling + Reward, data = data, family = "binomial"))
easyLogis <- lapply(easyData, function(data) glm(Choice ~ Handling + Reward, data = data, family = "binomial"))

# Coefficients
waitLogis_coeffs <- t(sapply(waitLogis, "[[", "coefficients"))
cogLogis_coeffs <- t(sapply(cogLogis, "[[", "coefficients"))
physLogis_coeffs <- t(sapply(physLogis, "[[", "coefficients"))
easyLogis_coeffs <- t(sapply(easyLogis, "[[", "coefficients"))
allLogis_coeffs <- rbind(waitLogis_coeffs,
                         cogLogis_coeffs,
                         physLogis_coeffs,
                         easyLogis_coeffs)
  
# Rank sums
rankHand <- wilcox.test(allLogis_coeffs[,2])
rankReward <- wilcox.test(allLogis_coeffs[,3])

# Plot coeffs
temp <- melt(allLogis_coeffs[,2:3])
qplot(data = temp, x = Var2, y = value, geom = "boxplot") + 
  labs(title = "Handling and Reward Coefficients", x = "", y = "Coefficients") +
  geom_hline(yintercept = 0, alpha = 0.5) +
  theme_classic()

rm(waitLogis, cogLogis, physLogis, easyLogis)
...
``` {r 16.1.2., echo=FALSE, fig.align="center"}
# Logistic fits
waitLogis <- lapply(waitData, function(data) glm(Choice ~ Handling + Reward + AR, data = data, family = "binomial"))
cogLogis <- lapply(cogData, function(data) glm(Choice ~ Handling + Reward + AR, data = data, family = "binomial"))
physLogis <- lapply(physData, function(data) glm(Choice ~ Handling + Reward + AR, data = data, family = "binomial"))
easyLogis <- lapply(easyData, function(data) glm(Choice ~ Handling + Reward + AR, data = data, family = "binomial"))
...
#waitLogis_coeffs <- sapply(waitLogis, function(data) (confint(data)[4,1] < 0 & confint(data)[4,2] > 0))
  
# Rank sums
rankAR <- wilcox.test(allLogis_coeffs[,4])

# Plot coeffs
temp <- melt(allLogis_coeffs[,2:4])


# The ugliest code, to check the existence of 0 on the CIs
AR_CIs <- data.frame(Subj = NULL, Low = NULL, High = NULL)
temp <- data.frame(Subj = seq(nSubsW), Low = rep(-0.1,nSubsW), High = rep(0,nSubsW))
for (i in seq(length(waitLogis))) {
  test <- tryCatch(round(confint(waitLogis[[i]]), digits = 2)[4,], error=function(e){})
  if(!is.null(test)) {
    temp$Low[i] <- test[1]
    temp$High[i] <- test[2]
  }
}
AR_CIs <- rbind(AR_CIs, temp)
temp <- data.frame(Subj = seq(nSubsC), Low = rep(0,nSubsC), High = rep(0,nSubsC))
for (i in seq(length(cogLogis))) {
  test <- tryCatch(round(confint(cogLogis[[i]]), digits = 2)[4,], error=function(e){})
  if(!is.null(test)) {
    temp$Low[i] <- test[1]
    temp$High[i] <- test[2]
  }
...
AR_CIs <- rbind(AR_CIs, temp)
temp <- data.frame(Subj = seq(nSubsP), Low = rep(0,nSubsP), High = rep(0,nSubsP))
for (i in seq(length(physLogis))) {
  test <- tryCatch(round(confint(physLogis[[i]]), digits = 2)[4,], error=function(e){})
  if(!is.null(test)) {
    temp$Low[i] <- test[1]
    temp$High[i] <- test[2]
  }
...
AR_CIs <- rbind(AR_CIs, temp)
temp <- data.frame(Subj = seq(nSubsE), Low = rep(0,nSubsE), High = rep(0,nSubsE))
for (i in seq(length(easyLogis))) {
  test <- tryCatch(round(confint(easyLogis[[i]]), digits = 2)[4,], error=function(e){})
  if(!is.null(test)) {
    temp$Low[i] <- test[1]
    temp$High[i] <- test[2]
  }
...
AR_CIs <- rbind(AR_CIs, temp)
AR_CIs$High[is.na(AR_CIs$High)] <- 0.1
AR_CIs$AR_Effect <- apply(AR_CIs,1, function(x) !(x[2] <= 0 & x[3] >= 0))

# Around 10% seemed to have been influenced by previous quitting
propAR_effects <- (sum(AR_CIs$AR_Effect) / totalSubs) * 100

rm(temp,waitLogis, cogLogis, physLogis, easyLogis)

```

To measure the significance of each subject's autoregressive coefficient, we computed its CI and checked how many contained zero. By this measure, around `r round(propAR_effects)` percent of our participants seemed to have been influenced by recent quitting. However, the autoregressive predictor did not preclude the effect of the remaininig experimental parameters.
...
#         labs(title = "Total Earned per Group", y = "Total Earnings (Cents)", x = "") +
#         theme_classic()
...
earnFitplot <- ggplot(data = summaryData, aes(propComplete, Earnings)) + 
         ...
         xlim(0.2,1) +
         labs(title = "Earnings as a Function of Acceptances", x = "Proportion Accepted", y = "") +
         theme_classic()
...
\bigskip
  ...
  **16.1.4.**	*To determine the optimality of each group’s decisions, we will perform two-sided one-sample (*one-sample? wouldn't it be paired vs optimal?*) t-tests to see if the proportion of acceptances for each time/reward combination was significantly different from the optimal rate (see 11.2.). This will result in 36 independent tests (3 rewards amounts, 3 handling/travel time combinations, and 4 groups), so we will correct for multiple comparisons using False Discovery Rate (FDR).*

``` {r 16.1.4., echo=FALSE}
# Split the data frame into every combination of parameters of interest
splitData <- split(allProp2, list(allProp2$Reward, allProp2$Handling, allProp2$Group))

# Perform a paired t-test on every combination of parameters
splitData_tests <- sapply(splitData, function(data) t.test(data$propComplete, data$Optimal, paired = T)$p.value)

# Adjust for multiple comparisons using BY-FDR
...
Of the 36 tests, around `r splitData_prop` were significantly deviant from optimality. Most of these were from effortful groups, as the wait group just showed deviations for 2 seconds handling/4 cent, and 10 seconds handling/8 cent offers (note to self: think about a way to properly .
...
``` {r 16.3.1. RTs, echo=FALSE,fig.align="center",fig.width=5,fig.height=4}
# RTs
...
The analysis of variance was significant (F = `r round(Fval, digits = 2)`, p = `r Pval`, R-squared = `r Rsquared_aov`). Pairwise post-hoc permutations showed significantly greater values for the wait group versus the cognitive (p = `r permuteOCTest$WvC$Pval`) and easy (p = `r permuteOCTest$WvE$Pval`) groups, as well as significantly greater gammas for the physical group and the cognitive group (p = `r permuteOCTest$CvP$Pval`). These results suggest that participants engaged in cognitively effortful demands experience significantly lower opportunity costs than those whose demands involve physical effort and pure delay. The reason for this is unclear, and we will work on it in the near future. 
