...
#   This issue partially goes back to a simple fact: environmental reward rates are learned independently of choices.
#   Instead, per-second sims track the attained average.
  ...
  if (environment == "Poor") {
    ...
    for (run in seq(n)) {
      ...
      for (trial in sequence) {
        ...
        r <- trialRewards[[trial]]
        accept <- ifelse(r > OC, T, F)
        ...
        if (accept) {
          if (asymmetric) {
            ...
            p <- p + alpha * (r - OC) # could turn this into a function, so multiple models can be tested
            evol <- rbind(evol, c(r, trialTimes[[trial]], p, trial, accept, poorAccept[[trial]], "Poor"))
          }
        } else {
          if (asymmetric) {
