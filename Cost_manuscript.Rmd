...
```
  ...
  In order to test these qualitative observations, we computed the probability of accepting a trial with a mixed-effects logistic regression (see Methods for details). As predicted, and confirming previous results, the model showed significant main effects of handling time (global intercept = `r round(fixef(mixLogis_main$Wait)["(Intercept)"], digits = 2)`, $\beta$ = `r round(fixef(mixLogis_main$Wait)["Handling"], digits = 2)`, SE = `r round(summary(mixLogis_main$Wait)[[10]]["Handling", "Std. Error"], digits = 2)`, *p* `r report_p(summary(mixLogis_main$Wait)[[10]]["Handling", "Pr(>|z|)"])`) and offer amount ($\beta$ = `r round(fixef(mixLogis_main$Wait)["Offer"], digits = 2)`, SE = `r round(summary(mixLogis_main$Wait)[[10]]["Offer", "Std. Error"], digits = 2)`, *p* `r report_p(summary(mixLogis_main$Wait)[[10]]["Offer", "Pr(>|z|)"])`). In order to show the comparisons among all conditions, Figure X portrays the coefficients (color scale **maybe exponentiated coefficients to make it more intuitive?**), and p-values (numbers within squares) that resulted from switching the reference cost condition. Specifically, each entry shows how much more likely was the reference group to accept an offer than the comparison group. The matrix of coefficients conveys that the cognitive group was significantly more likely to accept any offer than the physical and wait groups, but not the easy group.
  We next examined whether choice behavior could be better explained by reward statistics alone, or if cost conditions changed the way handling time and offer amount interacted to influence choices. Figure X shows AIC and BIC values for models of increasing complexity, starting with an intercept-only configuration (see Methods for details). Here we focus on AIC, as both metrics yielded comparable results. As predicted, the a priori model with all main effects performed better than those which on a single parameter (a priori model $R^2$ = `r round(mixLogis_compare$AllMainRsq, digits = 2)`), as well as the model with main effects for handling time and reward amount (AIC~a priori~ = `r ME_evals$abicLogis_compare["AllMain", "AIC"]`, AIC~handling/reward~ = `r ME_evals$abicLogis_compare["HR", "AIC"]`, $\chi^2$(`r ME_evals$HRvMain[[7]][2]`) = `r ME_evals$HRvMain$Chisq[2]`, *p* `r report_p(ME_evals$HRvMain[[8]][2])`). However, against our prediction, the a priori model was outperformed by models that added an interaction between handling and reward (AIC~HR interaction~ = `r ME_evals$abicLogis_compare["HR_interaction", "AIC"]`, $\chi^2$(`r ME_evals$MainvHRint[[7]][2]`) = `r ME_evals$MainvHRint$Chisq[2]`, *p* `r report_p(ME_evals$MainvHRint[[8]][2])`) as well as one that considered all possible interactions (AIC~all interactions~ = `r ME_evals$abicLogis_compare["Interaction", "AIC"]`, $\chi^2$(`r ME_evals$MainvAllint[[7]][2]`) = `r ME_evals$MainvAllint$Chisq[2]`, *p* `r report_p(ME_evals$MainvAllint[[8]][2])`). These improvements were driven by the fact that differences in offer acceptances among costs were never present for 20 cents (all *p* values for interactions containing offer amount < 0.001). As the highest reward should always be accepted in prey foraging environments, we conclude that the most parsimonious explanation of choice behavior comes from the a priori model containing all main effects. In combination with the pattern observed in Figure X, these results confirm that handling and reward amounts were similarly integrated across groups, but that the rate of acceptances was affected by the cost faced (and highest for the cognitive effort group).
