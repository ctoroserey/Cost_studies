...
## UPDATE TO THIS
# IT BASICALLY SAYS THAT PEOPLE DO TRACK THE RECENT OFFERS A BIT, BEYOND WHAT H+R DO.
# AND ALSO THAT RECENT FORCED TRAVELS + QUITS DO MARGINALLY INFLUENCE CHOICE
# THIS SUGGESTS THAT WHILE THE EFFECTS ARE SMALL (P(A) ~ 2%), PARTICIPANTS DO SOME ENVIRONMENTAL TRACKING
# history
...
historySize <- lapply(seq(2, 6), function(tail) {
  dat <- dataBtw %>%
  group_by(SubjID) %>%
  ...
  #summarise(aic = AIC(glmer(Choice ~ Handling + Offer + recentTravels + recentOffers + (1 | SubjID), data = ., family = "binomial")))
})

# then get the summary of the tail length that reduced the AIC the most
index <- which(sapply(historySize, function(iter) {AIC(iter)}) == min(sapply(historySize, function(iter) {AIC(iter)})))
summary(historySize[[index]])
```
