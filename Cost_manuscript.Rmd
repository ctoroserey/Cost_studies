...
### **Computational Modeling of Foraging Behavior**
  ...
  **GARY EXPLAINS FITTING PROCEDURE**
...
\begin{align}
  Model_{biased} = R_i - (\gamma + mC_i) H_i
\end{align}
...
\begin{align}
  Model_{adaptive + biased} = R_i - (\gamma_i + mC_i) H_i
\end{align}
...
``` {r OLD BTW and WTH: 16.3. OC modeling btw, echo = FALSE, fig.align = "center", fig.width = 6, fig.height = 4}
# ## fit the OC model
# summaryOC_btw <- list()
...
``` {r OLD BTW and WTH: 16.3. OC comparisons btw, echo = FALSE, fig.align = "center", fig.width = 6, fig.height = 4}
# # ANOVA
# OCanova <- list()
...
``` {r OLD BTW and WTH: 16.3. corrplot btw, echo = FALSE, fig.align = "center", fig.width = 5, fig.height = 5}
...
``` {r OLD BTW and WTH: 16.3. OC cross-validation btw, echo = FALSE, fig.align = "center", fig.width = 6, fig.height = 4}
...
``` {r OLD BTW and WTH: 16.3. OC Modeling wth, echo = FALSE, fig.align="center", fig.width=6, fig.height=4}
...
``` {r OLD BTW and WTH: 16.3. cross-v wth, echo = FALSE, fig.width=5, fig.height=4, fig.align="center"}
# # subdivide data
# dataTrain <- dataWth %>%
#    filter(Block < 5)
# 
...
``` {r OLD BTW and WTH: 16.3. param compare wth, echo = FALSE}
# # repeated measures anova
# OCs_anova <- list()
...
``` {r OLD BTW and WTH: 16.3. corrplot wth, echo = FALSE, fig.align="center", fig.width=5, fig.height=5}
...
# cost bias plot
costPlot <- ggplot(aovShape_btw$cost, aes(Group, Fit, fill = Group)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_boxplot(show.legend = F) +
  geom_jitter(width = 0.1, alpha = 0.5, show.legend = F, pch = 21, size = 3) +
  labs(x = "") +
  #facet_wrap(vars(Var1), scales = "free") +
  scale_x_discrete(labels = c("Wait", "Cognitive", "Physical")) +
  scale_fill_manual(values = colsBtw) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
...
allFitsPlot <- rbind(t1, t2) %>%
  ...
  mutate(Group = factor(Group, levels = c("wait", "cogTask", "phys", "Within", "pheasy"))) %>%
  ggplot(aes(Group, Fit, fill = Group)) +
    geom_boxplot(show.legend = F) +
    geom_jitter(width = 0.1, alpha = 0.5, show.legend = F, pch = 21, size = 3) +
    labs(x = "") +
    scale_x_discrete(labels = c("Wait", "Cognitive", "Physical", "Within")) +
    geom_vline(xintercept = 3.5, linetype = "dashed") +
...
```
  ...
  The fourth model produced the lowest BIC in both experiments (Figure 10A. Between-subjects = $modelComparison[4, 3]$; Within-subjects = $modelComparison[8, 3]$). Figure 10B-C show the key parameter fits from both experiments. Among these, the learning rate of the opportunity cost ($\alpha$) and the bias imposed by each demand on its estimate ($C_{demand}$) were of particular interest. Analyses of variance showed significant differences only for $c_{demand}$ (F(`r aovBtw$cost[[1]]$Df[1]`, `r aovBtw$cost[[1]]$Df[1]`) = `r fitFvals["cost"]`, *p* `r report_p(fitPvals["cost"])`), with $\alpha$ being uniformly low for all groups. Pairwise post-hoc permutations confirmed that the cognitive group (mean = `r round(mFits_btw["cost", "cogTask"], digits = 2)`, SD = `r round(sdFits_btw["cost", "cogTask"], digits = 2)`) had lower biases than either wait (mean = `r round(mFits_btw["cost", "wait"], digits = 2)`, SD = `r round(sdFits_btw["cost", "wait"], digits = 2)`; *p* `r report_p(fitPerms["Cognitive", "Wait"])`; Cohen's D = `r fitES["Cognitive", "Wait"]`) or physical (mean = `r round(mFits_btw["cost", "phys"], digits = 2)`, SD = `r round(sdFits_btw["cost", "phys"], digits = 2)`; *p* `r report_p(fitPerms["Cognitive", "Physical"])`; Cohen's D = `r fitES["Cognitive", "Physical"]`) groups. These biases were significantly higher than zero for the wait group (t(`r fitOStest$tresults[[1]]$parameter`) = round(`r round(fitOStest$tresults[[1]]$statistic, digits = 2)`, *p* `r report_p(fitOStest$tresults[[1]]$p.value)`), and lower than zero for the cognitive group (t(`r fitOStest$tresults[[2]]$parameter`) = round(`r round(fitOStest$tresults[[2]]$statistic, digits = 2)`, *p* `r report_p(fitOStest$tresults[[2]]$p.value)`). This suggests that rather than reducing costs, cognitive tasks boosted the value of their concommitant offers. 
  
  The median values of $c_{demand}$ from each group were carried over to the within-subjects fitting, and were aggregated over time according to a weighted decay into a final $C$ value (see Methods for details). As can be seen on Figure 10C, parameter values were comparable across experiments, pointing to consisency in the way decision makers approached our foraging scenarios. In addition, values of $\alpha_w$ suggested that $c_{demand}$ estimates converged towards the second half of the experiment (mean = `r mFits_wth[4]`, SD = `r sdFits_wth[4]`), gradually reducing bias by the end of the experimental session (mean = `r mFits_wth[3]`, SD = `r sdFits_wth[3]`). Model fits also provided good qulitative resemblance to the observed choice behavior in both experiments, which can be found on **Supplemental Figure X** .
  
```{r BIC (Fig. 10), eval = !figsEnd, fig.align = "center", fig.height = 5, fig.width = 7, fig.cap = "A. BIC values for four plausible models of choice behavior for both experiments. B. Differences in cost biases produced by each demand. The cognitive group acted as if it perceived its demand as valuable. C. Parameter comparison across experiments."}
...
- Being immersed in a task can make time transpire subjectively faster [@Csikszentmihalyi2014], which might have made the cognitive task attractive. Time sensitivity has been previously used to improve fits on classic hyperbolic models of delay discounting [@Mckerchar2009].
