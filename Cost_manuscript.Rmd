...
## Discussion
  ...
  Understanding the sporadic attractiveness of effortful options remains elusive. In this study, we tested the hypothesis that such appeal could depend on the ability to achieve the same outcome through different means (**not quite**). Using two foraging experiments that exposed individuals to a single or intermixed types of demands, we found that 1) preferences for cognitive effort were stable when individuals face a single form of demand, but fade over time when other costs are also experienced; and 2) the phenomenon can be explained by a model in which preferences depend on how participants construe the amount of time a task takes, which could hypothetically be recalibrated when all tasks are interleaved.
  
  Our findings provided partial support for our pre-registered hypotheses, which can be found in https://osf.io/2rsgm/. In accord with our predictions, decision makers were successful in approximating reward-maximizing strategies, as evidenced by acceptance rates. This is in line with previous foraging experiments, where the main deviation from optimality is a tendency to overharvest. Unlike our first hypothesis, our predictions regarding differences among costs did not
