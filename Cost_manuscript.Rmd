...
\end{aligned}$$
  ...
  where $\gamma$ was the mean total earning rate for the whole group (**ADD VALS**). At each decision point, $mC$ was the bias produced by each demand $c_{demand}$ weighted against a global bias estimate $C$
...
#####
  ...
  We therefore added a time sensitivity parameter $S$ to the model, with a separate value associated with each type of demand. The values of $S_{demand}$ could then be recalibrated as participants experienced them, averaged according to the following formula [adapted from @Sutton2018]
