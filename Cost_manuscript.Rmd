...
author: "Claudio Toro-Serey, Gary Kane, & Joseph T. McGuire"
header-includes:
...
## Introduction
  ...
  Evaluating whether to pursue a desired outcome often requires assessing the demands required for obtaining it. In psychology and economics, demands for both time and effort have traditionally been identified as costs that detract from the net subjective value of rewards (Westbrook & Braver, 2015). Indeed, decision makers tend to prefer low-effort and immediate rewards over larger rewards that demand longer delays and higher effort (Ainslie, 1975; Hull, 1943; Kool & Botvinick, 2018; Kool, McGuire, Rosen, & Botvinick, 2010; Treadway, Buckholtz, Schwartzman, Lambert, & Zald, 2009; Westbrook, Kester, & Braver, 2013). However, people often change their attitudes towards exerting effort in everyday decisions, resulting in pursual of effortful avenues instead. Emerging experimental and theoretical work has chiefly explained this phenomenon by suggesting a reciprocal relationship between effort and outcome, whereby exerting effort boosts the perceived value of its outcomes (Kacelnik and Marsh, 2002; Mochon, Norton, and Ariely, 2012; Hernandez Lallement et al., 2014), and positive outcomes imbue value into their preceding effortful behavior (i.e. learned industriousness, Eisenberger, 1992). Though intuitively valid and empirically supported, this explanation leaves open questions about what features of an effortful task can make it attractive in its own right, and how contextual factors mold its perceived costs (Inzlicht, Shenhav, and Olivola, 2018). Understanding these intrinsic qualities could improve our ability to motivate completion of effortful daily activities, such as schoolwork or physical exercise.
  
  What is often missing from existing accounts is the common case of choosing among types of behavioral demands. Studies have traditionally compared the reduction of subjective value imposed by each type of demand, often through discounting functions that capture how much less appealing a reward becomes as a function of the amount or intensity of its associated behavior (Ainslie, 1975; Frederick et al., 2002; Kable and Glimcher, 2007; Kool et al., 2010; Westbrook, Kester, and Braver, 2013; Green, Fristoe, and Myerson, 1994). Although this line of work suggests that discounting functions are similar for delay and effort (Prevost et al., 2010; Massar et al., 2015; Seaman et al., 2018), there is ongoing debate about the shape of the discounting function across effort types (Arulpragasam et al., 2018; Prevost et al., 2010; Chong et al., 2016; Klein-Flugge et al., 2016; Kool and Botvinick, 2018), and discount rates do not always correlate across cost domains (Westbrook and Braver, 2013; Seaman et al., 2018). However, sporadic preferences for effort could arise from the availability of other types of demands in the same choice environment—in other words, from its context (Kool & Botvinick, 2018). For example, exerting high effort for a reward might be aversive, but the same level of work might become valuable when compared to the boredom of passive waiting (e.g. taking the cumbersome way home rather than waiting for a delayed train, even if the resulting time of arrival is the same). Conversely, even though people tend to donate more to charitable events that require them to perform effortful activities (i.e. the martyrdom effect), they are also less likely to engage in such events once an easier alternative to raise funds becomes available (Olivola and Shafir, 2013). Recent work has proposed that the subjective costs produced by delays and cognitive effort is modulated by the possibility of missing out on better concurrent alternatives, namely the opportunity cost of time (OC; Fawcett et al., 2012; Kurzban et al, 2013; Otto and Daw, 2019). This evidence sets the stage for examining what people would actually select when given the option for multiple types of demands, including the unexplored question of deciding between cognitive and physical effort (Inzlicht, Shenhav, and Olivola, 2018).

  Single choice foraging paradigms provide a natural way to probe preferences among demands, by asking individuals whether the current offer outweighs what could be obtained and experienced during that time instead (Stephens & Krebs, 2019). Foraging has recently attracted high scientific interest due to its ecological validity, which is rooted in evolutionary choice behaviors (Hayden 2018; Mobbs, Trimmer, Blumstein, & Dayan, 2018), and offers a number of desirable qualities. First, it allows for the estimation of optimal, reward maximizing choice patterns based on the opportunity cost of time (Charnov, 1975; Krebs et al., 1977). We can therefore interpret observed deviations in acceptance rates from such optimal behavior in addition to comparing them among demands. Second, the sequential nature of foraging allows individuals to emphasize different elements of the decision context. For example, the cost (or value) of effort or delay could globally depend on the availability of alternative demands (as suggested above), impacting the perceived opportunity cost of time. Alternatively, it could be intrinsic and robust to such manipulation, and/or depend on task-specific factors that are adjusted once individuals experience different forms of behavioral costs. Foraging rests on an abundant modeling history that can help to computationally capture such emphases (Stephens & Krebs, 2019).

  In this study, we tested the hypothesis that preferences would vary depending on whether individuals face a single or multiple types of behavioral demands (i.e. cognitive effort, physical effort, and delay). We explored this question in two foraging experiments: a between-subjects design in which participants foraged for rewards while performing a single demand, and a within-subjects extension that had individuals forage for demands in addition to rewards. We leveraged the ecological and computational richness of foraging paradigms to provide new insights on a number of open questions (Inzlicht, Shenhav, and Olivola, 2018). First, the existence of a well-defined optimal choice strategy helped us identify when demands boost the value of rewards (i.e. overharvesting poor offers). Second, increasing the level of exposure to types of demands allowed us to asses whether their cost was intrinsic or relative. Finally, confronting participants with both mental and physical effort let us probe whether these preferences were domain general or task-specific, and in the case of the latter, isolate factors that influenced preferences beyond the often-cited associative relationship between effort and outcome.
...
### **Computational Modeling of Foraging Behavior**
  ...
  We started by fitting a base model derived from the normative predictions of the Marginal Value Theorem (MVT; Charnov, 1975), which states that a delayed reward should be accepted if its earning rate surpasses the opportunity cost (OC) of time incurred in obtaining it. This decision rule can be expressed as
...
\end{aligned}$$
  ...
  The base formulation lacked the flexibility needed to capture the dynamics observed in the within-subjects experiment. We therefore tested a second model which allowed $\gamma$ to evolve over time, adapting a learning rule from Constantino & Daw (2015)
...
\end{aligned}$$
  ...
  in which high values of the free parmeter $\alpha_w$ denoted that decision makers incorporated the recalibrated subjective time early on in the experiment. Similar to McKerchar et al. (2009), the values of $mS_i$ exponentially modulated every instance of the handling time ($H_i$) from the adaptive model. In this way, subjective time perception produced by different demands could influence choice in two ways: retrospectively affecting the value of $\gamma$ through the perceived length of recent trials
...
```{r Prepost mixed effects plot (Fig. 6), fig.align = "center", fig.cap = "Distilling acceptance behavior before and after the break. Left: Proportion of acceptances per handling, reward, and half, showing that effortful demands became increasingly aversive over time. Right: Mixed-effects coefficients denoting the comparison among costs acceptances for each half separately (mirrored along the diagonal). Even though acceptances steadily decreased in the effort groups, the relative preferences were preserved."}
...
## Discussion
  ...
  Understanding the sporadic attractiveness of effortful options remains elusive. In this study, we tested the hypothesis that such appeal could arise from being able to compare different means to achieve the same outcome. Using two foraging experiments that exposed individuals to a single or intermixed types of demands, we found that 1) preferences for cognitive effort were stable when individuals face a single form of demand, but fade over time when other costs are also experienced; and 2) the phenomenon can be explained by a model in which preferences depend on how participants construe the amount of time a task takes, which could hypothetically be recalibrated when all tasks are interleaved.
  
  Rather than comparing discounting functions among demands, allowing people to choose for them helped us control for a number of confounds. For example, effortful tasks often take longer to perform in everyday life. We eliminated this possibility by using a common handling time for all demands. 
  
  The observed choice dynamics ruled out a number of explanatory candidates. For instance, differences in choice stability between experiments suggest that a demand's cost is not intrinsic, while differences between effort types rule out learned industriousness. Effort preferences due to boredom would not have shown the global acceptance decline and convergence in the within-subjects experiment, which also discards the possibility that all our participants were high on need for cognition (CITE). Even though fatigue could explain the decrease in effort acceptances across experiments, we would have expected tired participants to turn to wait trials over time in order to preserve earning rates. We also ruled out the sunk cost effects used to explain overharvesting (Wikenheiser2013; Sweiss2018), as participants expressed their choices as early as possible, resulting in occassional underharvesting. Even though the reciprocal relationship between effort and outcome can often explain preferences for effort (Inzlicht2018), the observed differences in acceptances between effort types and the eventual convergence of rates in the within-subjects study cannot be explained by reward alone, as demands were rewarded at the same rate. Instead, we propose that features of the cognitive tasks added outcome-independent value to the offer. Subjective time sensitivity has been shown to play a role in temporal discounting, improving on classic hyperbolic fits (Mckelachar....) *add that stability meant no need to model asymmetric learning across environmental richness from Neal* (unlikely to be need for cognition bc of exp 2 global decrease)
  
# Points of discussion 
- We still think that cognitive and physical manipulations were effortful. Our physical effort task has been repeatedly used to probe effort. Similar task switching has been shown to feel cognitively effortful (Kool). Furthermore, the effortful nature of our manipulations is evidenced by the steeper decrease in acceptances over time at longer handling times (Treadway). There are many reasons why such cognitive manipulations can be effortful, which are often centered in the amount of control. Or the OC. 
- Note that the calibration of efforts were different. We adjusted to people's physical capabilities, but trained them in the cognitive tasks until they reached criterion. 
- Efficacy, along with reward amount, can influence how much control is exherted.
- But here we do not focus on choosing the *amount* of cognitive control to be exherted during performance, which can be due to efficacy, agency, or reward amount. Instead, we focus on what makes people *pursue* the task in spite of how effortful it is.
- Clinical: such deviations in the willingness to pursue effort can predict depression relapse (https://jamanetwork.com/journals/jamapsychiatry/article-abstract/2761562), and motivational factors affecting effort aversion can be heterogenous between depression and schizophrenia (https://www.researchgate.net/publication/337361164_Pathways_to_Motivational_Impairments_in_Psychopathology_Common_Versus_Unique_Elements_Across_Domains). Like, the aversion is due to focusing on different elements of the task.
- Having a between-subjects experiment provided a baseline of foraging behavior that helped us interpret the findings of the second experiment
- Control: for time as confound of effort
- Control: having two efforts helped dissociate domain-general from task-specific features.
- Control: participants were fully aware. This removed the possibility of asymmetric learning and choice difficulty.
- Control: in demand experiments, chosen options are often carried out after the experiment, but there are known discrepancies between expected and obtained value that could affect repeated decisions in real life.

- Control: even in TAFC tasks, selecting the shorter option could index
- Optimality let us know that people acted as if effort was attractive. This was evidenced by both the lower earnings by the cognitive effort group, and by the base OC model. Of note is that the optimality of the strategies produced by each cost depended on the richness of the block. This can help explain why the wait group earned the most despite having similar acceptance rates to the physical group, as the former was closer to optimality in the richest environment.
- This appeal couldn't be explained by a number of explanatory candidates
- This includes the often-cited reciprocal relationship between effort and reward
- But it turns out subjective time fits the bill. 
- Cognitive effort gained value by making trials feel shorter
- This could be because: 1) discrete nature of cog taska llowed for counting; 2) flow; 3) persistent recruitment of working memory (Buhi and Gershman). Talk about future experiments: if this is subjective time, changing the pacing of the tasks should change the acceptance rates for cognitive effort.
- maybe: gifts are valuable more when it required effort to acquire them.





  The observed choice dynamics ruled out a number of explanatory candidates. For instance, differences in choice stability between experiments suggest that a demand's cost is not intrinsic. Effort preferences due to boredom would not have shown the global acceptance decline and convergence in the within-subjects experiment, which also discards the possibility that all our participants were high on need for cognition (CITE). Even though fatigue could explain the decrease in effort acceptances across experiments, we would have expected tired participants to turn to wait trials over time in order to preserve earning rates. We also ruled out the sunk cost effects used to explain overharvesting (Wikenheiser2013; Sweiss2018), as participants expressed their choices as early as possible, resulting in occassional underharvesting. Even though the reciprocal relationship between effort and outcome can often explain preferences for effort (learned industriousness, Inzlicht2018), the observed differences in acceptances between effort types and the eventual convergence of rates in the within-subjects study cannot be explained by reward alone, as demands were rewarded at the same rate. Instead, we proposed that subjective time sensitivity has been shown to play a role in temporal discounting, improving on classic hyperbolic fits (Mckelachar....) *add that stability meant no need to model asymmetric learning across environmental richness from Neal* (unlikely to be need for cognition bc of exp 2 global decrease)
  
  
- Things that don't seem to be at play here, but could still play a general role in mental effort preferences: 1) A demand’s cost is not fully intrinsic (within vs between differences); 2) Effort preferences unlikely just due to learned industriousness or efficacy as amitais lab sees it (differences between effort types, and everyone met a good performance level per training); 3) avoiding boredom (global decline in exp 2, not a preference exchange). 4) Literally counting tasks, since mental effort was discretely presented but everything else was contiguous. If that were the case, dividing the handling time by 2 would work, but not sure if it would translated into exp 2. 5) fatigue: there's a similar gradual decrease in acceptances in both experiments, except for the wait condition in exp 1. Fatigue would apply equally to both, or alternatively it might not bring all down to the same level (I guess the point could be made, but it's not that people who complete more at the beginning decrease acceptances more, as one would expect from fatigued people). Also worth mentioning that even if this is due to need for cognition (i.e. Ceyda; Laura; Cacioppo et al 1996), the collapsing in the second experiment shows that preferences for cognitive effort cannot be just that.
