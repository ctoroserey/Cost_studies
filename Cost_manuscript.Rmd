...
where $A_i$ is an acceptance indicator (0 or 1), and $\tau_i$ the amount of time spent in the trial. Higher values of the learning rate $\alpha$ (ranging from 0 to 1) prioritized updating from the current trial’s earning rate, weighed by the amount of time experienced. This addition allowed for the tracking of recent history that was part of our criteria. 
  ...
  We further parameterized this model to test the hypothesis that demands affected the perceived duration of the handling time differently, thus potentially improving their appeal (e.g. a sense of fast completion might have made the cognitive task attractive). Time sensitivity has been previously used to improve fits on classic hyperbolic models of delay discounting [Mckerchar2009]. Similar to that work, we added an exponential parameter $S$ to all instances of the handling time ($H_i$) handling time, with a value of $S$ associated with each type of demand. The values of $S$ could then be aggregated across demands as participants experienced them, according to the following formula [adapted from @Sutton2018]
  
  $s_{i + 1} = s_i + A_i \left (\frac{1}{i}[S_i - s_i] \right)$

  in which the global value of $s$ was updated every time a decision maker engaged with a trial. We fitted the starting value of the a demand's parameter $S$, which was bounded between 0 and 2 (values lower and greater than 1 indicating slower and faster perceived time, respectively). We assumed that each participant would incorporate t$s$ into their decisions at different rates
  
   $mS_i = (s_i w_i) + (S_i (1 - w_i))$

  such that in every trial, the recalibrated estimate of how long the handling time feels ($s$) is balanced against the actual experience of subjective time produced by the incoming demand ($S$). The weight $w_i$ controls how early participants start integrating the reclalibrated time $s$ into their decisions, given by
