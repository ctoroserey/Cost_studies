...
\end{align}
  ...
  A third model considered the possibility that each type of demand additively biased the estimate of $\gamma$. Such bias would be stable if a single demand was experienced, but recalibrated into a global estimate as decision makers faced different types of demands. The overall model took the following form
  
\begin{align}
  Model_{biased} = R_i - (\gamma + mC_i) H_i
\end{align}
  
  where $\gamma$ was the mean total earning rate for the whole group (0.55). At each decision point, $mC$ was the bias produced by each demand $c_{demand}$ weighted against a global bias estimate $C$
  
\begin{align}
  mC_i = C w_i + c_{demand} (1 - w_i)
\end{align}
  
  with $w_i$ controlling how early participants started integrating the converging bias $C$ into their decisions. The value of this weight was given by
...
bic_plot <- ggplot(data = modelComparison, aes(Model, BIC, group = Exp, color = Exp)) +
  geom_line(size = 2) +
  geom_point(size = 4) +
...
```
  ...
  We tested four models based on a set of criteria derived from observed choice behavior (see Methods for details). The first one, a base formulation, probed the predictions of the Marginal Value Theorem (Charnov, 1975). The second one allowed the opportunity cost to gradually adapt using a reinforcement learning rule. The third model assumed that different demands imposed distinct biases on the perceived opportunity costs, and that these biases could be recalibrated with experience. A final model combined the adaptive features of the second and third formulations.
