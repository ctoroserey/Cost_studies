...
**new** The adaptive nature of choices in this experiment demanded more flexibility than the single-parameter model fitted above. Based on the results, an adequate model of behavior should meet X criteria: 1) an initial preference for cognitive effort trials 2) that this preference disappeared as acceptances devolved to a common rate across costs; 3) show that participants kept track of recent offer history; 4) that these effects were robust to block order effects; 5) deviations from optimality that were not simply due to overharvesting; and 6) be applicable to both experiments. Alternative foraging models from the literature were not fitted, as they did not qualitatively meet these criteria (wikenheiser, C&D, Fawcett...)
...
historySize <- lapply(seq(2, 7), function(tail) {
  dat <- dataBtw %>%
    #filter(Cost != "Wait") %>%
    group_by(SubjID) %>%
...
historyIndex <- which(sapply(historySize, function(iter) {AIC(iter)}) == min(sapply(historySize, function(iter) {AIC(iter)})))

historyCoeffs <- round(summary(historySize[[historyIndex]])$coefficients, digits = 2)
...
```
  ...
  We performed a mixed effects logistic regression to address the first hypothesis. Since participants in the effort groups were forced to travel when they failed to perform above threshold (see Methods), we distinguish between acceptances (entering the handling period) and completions (successfully obtaining a reward). Larger reward amounts significantly increased acceptance probabilities ($\beta$ = `r historyCoeffs["Handling", "Estimate"]`, SE = `r historyCoeffs["Handling", "Std. Error"]`, *p* `r report_p(historyCoeffs["Handling", "Pr(>|z|)"])`), whereas longer delays decreased them ($\beta$ = `r historyCoeffs["Offer", "Estimate"]`, SE = `r historyCoeffs["Offer", "Std. Error"]`, *p* `r report_p(historyCoeffs["Offer", "Pr(>|z|)"])`), in line with predictions. Two additional regressors showed that having missed out on consecutive rewards decreased the probability of acceptance ($\beta$ = `r historyCoeffs["recentTravels", "Estimate"]`, SE = `r historyCoeffs["recentTravels", "Std. Error"]`, *p* `r report_p(historyCoeffs["recentOffers", "Pr(>|z|)"])`), and that participants became more selective when recent offer history was richer ($\beta$ = `r historyCoeffs["recentOffers", "Estimate"]`, SE = `r historyCoeffs["recentOffers", "Std. Error"]`, *p* `r report_p(historyCoeffs["recentOffers", "Pr(>|z|)"])`). The reduction in acceptances from recent misses was driven by the wait group (removing this group from the model yielded a *p* > 0.1), likely because this group accepted the least amount of offers. Comparing likelihood fits among lengths of offer history (e.g. sum over 2 or 3 past trials) suggested that participants were influenced by the `r historyIndex + 1` most recent rewards (AIC = `r AIC(historySize[[historyIndex]])`), which matched the offer presentation rotation (see Methods). These results confirm the expected influence of time and offer magnitude on choice behavior, but also suggest that participants adapted to their recent experience.
...
- Working memory could produce timing issues (Buhi n Gershman). Efficiency couldn't be it because then global decrease wouldn't happen and phys effort might be affected too (Shenhav stuff). 
