...
```
  ...
  We tested four models against a set of criteria derived from observed choice behavior (see Methods for details). The first one, a base formulation, probed the predictions of the Marginal Value Theorem (Charnov, 1975). The second one allowed the opportunity cost to gradually adapt using a reinforcement learning rule. The third model assumed that different demands imposed distinct cost biases on the perceived opportunity costs, and that these biases could be recalibrated with experience. A final model combined the adaptive features of the second and third formulations.
