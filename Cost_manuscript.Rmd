...
paramPlot <- ggplot(param, aes(Group, Fit, fill = Group)) +
    ...
    # geom_hline(yintercept = 0.7, alpha = 0.9, color = "gray40", size = 1) +
    # geom_hline(yintercept = 0.9, alpha = 0.9, color = "gray60", size = 1) +
    #geom_hline(yintercept = 0, alpha = 0.9, size = 1, linetype = "dashed") + # mean of highest earning rates across blocks
    #geom_dotplot(stackdir = 'center', binaxis = 'y', show.legend = F) +
    geom_boxplot(show.legend = F) +
    geom_jitter(width = 0.1, alpha = 0.5, show.legend = F, pch = 21, size = 3) +
    #ylim(0,1.5) +
...
```
  ...
  The fourth model produced the lowest BIC in both experiments (Figure 10A. Between-subjects = $modelComparison[4, 3]$; Within-subjects = $modelComparison[8, 3]$). Figure 10B shows the parameter fits from the between-subjects experiment. Among these, the learning rate of the opportunity cost ($\alpha$) and the bias imposed by each demand on its estimate ($C_{demand}$) were of particular interest. Analyses of variance showed significant differences only for $c_{demand}$ (F(`r aovBtw$cost[[1]]$Df[1]`, `r aovBtw$cost[[1]]$Df[1]`) = `r fitFvals["cost"]`, *p* `r report_p(fitPvals["cost"])`), with $\alpha$ being uniformly low for all groups. Pairwise post-hoc permutations confirmed that the cognitive group () had lower biases than either wait () or physical () groups. Furthermore, these estimates were significantly lower than zero (tstats). These fits suggest that individuals integrated the opportunity cost of time at similar rates regardless of the concomitant demand, but that cognitive tasks boost the value of offers (**reword**).
